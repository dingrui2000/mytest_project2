...
class ClipGraph(ModelBase):
    ...
    def embed_text(self,
        ...
        embeds = outputs.last_hidden_state  # 这里是关键的修改
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor,
        ...
        lm_embeds = lm_embeds.mean(dim=1)  # 对所有嵌入求平均
        lm_embeds = F.normalize(lm_embeds, p=2, dim=1)
        lm_embeds = self.dropout(lm_embeds)
        assert lm_embeds.shape[1] == self.embed_dim  # 检查
        print("Shape of lm_embeds:", lm_embeds.shape)
        print("Value of self.embed_dim:", self.embed_dim)
        ...
        assert gnn_embeds.shape[1] == self.embed_dim #检查
        ...
        assert ret.shape[0] == lm_embeds.shape[0] #检查
        assert ret.shape[1] == gnn_embeds.shape[0] #检查
        print("Shape of logits (ret):", ret.shape)
